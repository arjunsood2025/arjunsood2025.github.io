...
Built a market-data research platform ingesting exchange L2 order-book/trade feeds into hive-partitioned Parquet with schema validation, sequence-gap detection, and DuckDB SQL achieving 2.5–2.9× faster analytical queries than a pandas baseline via partition pruning and predicate pushdown.

* Wrote a C++20 limit-order-book replay engine with zero-copy pybind11/NumPy bindings (GIL released in the hot loop), processing 11.5M events/sec — \~488× the pure-Python reference — with bit-identical cross-language parity enforced in CI.
...
* Shipped production tooling around the research core: FastAPI experiment service, SQLite run registry, Streamlit dashboard, seeded synthetic market simulator for full reproducibility, 55 tests, GitHub Actions CI, and multi-stage Docker builds.
...
* Engineered a causal microstructure feature store (order-flow imbalance, book imbalance, realized volatility, rolling z-scores) with machine-checked no-lookahead guarantees: a truncation-invariance test asserts features are bit-identical when future data is removed, and the TCN normalizes per timestep so normalization cannot pool statistics across the time axis.
* Designed a leakage-safe evaluation suite — purged/embargoed walk-forward splits, shuffled-label sanity tests, regime-sliced metrics, calibration curves, and transaction-cost stress tests — which showed the direction signal's Sharpe crossing zero at \~0.14 bps per trade and a 0.899 volatility AUC collapsing to 0.28–0.55 within regimes, i.e. mostly regime detection rather than skill.
* Identified that the project's headline baseline comparison was invalid (5-fold walk-forward scored against a single held-out split) and built a matched-split harness; on identical test windows logistic regression ties the transformer (0.738 AUC), showing the injected signal is linear and the deep models add nothing here.
* Deployed a versioned model registry (atomic promotion, scaler shipped with weights) behind a FastAPI service with Prometheus histograms; measured 1.9 ms p50 single-window CPU latency and 2,774 windows/s peak throughput with a p50/p95/p99 benchmark harness; 43 pytest tests cover data, models, evaluation, and API.

### LLM Pretraining, Post-Training, and High-Performance Inference Engine

Built an end-to-end LLM platform — from-scratch BPE tokenizer, LLaMA-style transformer (RoPE/GQA/SwiGLU), and a pretraining stack with DDP/FSDP2, bf16, and custom Triton kernels (fused RMSNorm, fused cross-entropy) — training a 125M model on 1.26B tokens at 42.7% MFU on a single RTX 5070 with bitwise-deterministic checkpoint resume (distributed paths implemented and unit-tested; benchmarked single-GPU).

* Ran a 7-run scaling study fitting compute–loss power laws against Chinchilla predictions (fitted exponent 0.195; Chinchilla joint fit giving a data exponent of 0.323 against the paper's 0.28), with matched-compute architecture ablations (RoPE, SwiGLU, GQA) and a MinHash-deduped, contamination-filtered data pipeline over memory-mapped shards.
* Post-trained via SFT (loss masking, sequence packing; 30k conversations) and a from-scratch DPO implementation (4k preference pairs, 58% preference accuracy, KL-controlled); evaluated across base/SFT/DPO with lm-eval-harness (HellaSwag, ARC, PIQA, WinoGrande), perplexity, and refusal probes, plus a position-debiased LLM-judge harness with bootstrap confidence intervals.
* Engineered a vLLM-style inference engine: paged KV cache with prefix caching and copy-on-write (170 vs 32 concurrent sequences, 5.3× contiguous KV), continuous batching with recompute preemption, speculative decoding (3.09 output tokens per target forward at 53% draft acceptance), and int8 weight-only quantization — served over a streaming SSE server with Prometheus metrics and p99 TTFT/ITL measured under open-loop Poisson load.
* Shipped with 65+ unit tests in CI (logit-parity between engine and reference model, paged-vs-dense attention equivalence, kernel correctness, bitwise training resume), Docker packaging, a model card, and a reproducible benchmark report against a naive static-batching baseline.
...
* Engineered a FoundationDB-style deterministic simulator — entire cluster on virtual time with seeded fault injection (partitions, crashes, message loss, forced snapshot transfers) and exact seed replay — executing 20,000 failure schedules (\~420M events, \~17.3M client ops) with a from-scratch Wing-Gong linearizability checker validating every run's client history.
* Caught and documented 8 real bugs pre-release (reproducing seeds preserved as CI regressions), including a state-machine safety violation in which a snapshot covering a log prefix truncated a follower's acked entries and a committed entry was later overwritten by a legitimate election — five rare events aligning at one instant, reached by exactly one seed in 20,000 and invisible to unit tests and live chaos testing alike.
* Shipped the production layer: tokio server with single-owner event loop, group commit (batches an entire event-loop drain into one fsync), exactly-once client sessions via replicated dedup, and three read-consistency modes — measured with the workload held fixed, linearizable reads cost \~1.5× stale/lease (9,043 vs \~13,000–13,500 ops/s) on a 3-node cluster.
* Measured failure behavior honestly: sub-second leader election under 300,000-op load with zero lost operations, client-observed failover decomposed (election vs. rediscovery) via a YCSB-style benchmark suite built for the project.
